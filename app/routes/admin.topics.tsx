...
export async function loader({ request }: { request: Request }) {
  ...
  if (pagination.search) {
    conditions.push(like(topic.title, `%${pagination.search}%`));
  }

  const whereClause = conditions.length > 0 ? and(...conditions) : undefined;

  const orderBy = pagination.order === "asc" ? asc(topic.created) : desc(topic.created);
  ...
  const [topics, [{ total }]] = await Promise.all([
    ...
    db.select({ total: count() }).from(topic).where(whereClause),
  ]);
...
export default function TopicsListPage() {
  const { topics, pagination } = useLoaderData<typeof loader>();
  ...
  const [confirmDialog, setConfirmDialog] = useState<{
    ...
    intent: string;
    topicId: number;
  }>({ open: false, title: "", description: "", intent: "", topicId: 0 });
  ...
  const handleConfirm = () => {
    const { intent, topicId } = confirmDialog;
    if (intent === "delete") {
      fetcher.submit({ intent: "delete", topicId: String(topicId) }, { method: "post" });
    }
  ...
  const columns: ColumnDef<TopicRow>[] = [
    ...
    },
    {
      ...
      header: "Views",
      cell: ({ row }) => (
        <span>{row.original.view.toLocaleString()}</span>
      ),
    },
    {
      accessorKey: "created",
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem
              ...
              onClick={() => setConfirmDialog({
                open: true,
                title: "Delete Topic",
                description: `Are you sure you want to permanently delete the topic "${row.original.title}"? This action cannot be undone.`,
                intent: "delete",
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage platform topics. {pagination.total.toLocaleString()} total records.
          </p>
        </div>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new topic for the platform.
            </DialogDescription>
          </DialogHeader>
